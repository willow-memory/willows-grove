...
| **Grove bus · Discord bridge ✓** | Not exercised tonight. Grove is gate-denied to the `jeles` seat, so a specialist cannot post its own work there. **Correction from the grove tree (2026-08-29):** Grove ships a **Matrix** bridge, not a Discord one — `bridge/matrix.py` plus an appservice `registration.yaml`. "Discord" survives in Grove only as a UI idiom (`grove_db.py:330` parses Discord-style `#fleet` channel names; `grove_reader.py:280` calls the member pane Discord-style). If v0.4 draws a Discord bridge on the Grove element, either the element is mislabelled or the bridge it means lives outside this repo. |
| **Daemons: gitsync 25min · kart workers ✓ · grove ✓ · discord ✓** | **One** unit running: `willow-mcp-worker-fast`. The rest are inactive. **Partial cause found in the grove tree (2026-08-29):** the grove daemons could not have been running as units — `deploy/` ships only `grove-mcp-serve.service.template`. The `grove-serve` and `grove-watcher` unit templates its own operator guide references, and the `scripts/grove-serve-run` / `scripts/grove-watcher-run` launchers, were never committed here (lost in the v0.9 clean-build port). Being fixed on `claude/ui-build-progress-5pyc41`. This explains the grove column only; the gitsync and kart rows are unexamined from here. |
