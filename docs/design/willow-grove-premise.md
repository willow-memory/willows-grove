...
| Ratatosk sessions | `willow-memory/ratatosk` — platform session runtime; Grove lifecycle via MCP when installed | **add-on** | listens when `RATATOSK_GROVE_CHANNEL` + `--mcp`; `--listen` for bus tasks |
...
| **`willow-memory` beyond charter** | `willow-data-vault`, `willow-gate`, `kartikeya`, **`ratatosk`**, `corpus-lens` | vault / gate / kart / **session runtime** / corpus surfaces when installed |
