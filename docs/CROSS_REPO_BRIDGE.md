...
| **System layer (KB, tasks, routing PG schema)** | `willow-2.0` | `pg_bridge.py` public schema, embeddings, Kart-facing tables, routing_decisions shape used by tools |

**Ground truth split:** Grove is the **human+agent message bus** and holds **`grove.messages`**. Willow owns **`public.knowledge`**, extended task/dispatch tables, and the richer **`routing_decisions`** row shape when both stacks share one Postgres (`willow_20`).

## Shared database

Both codebases assume (by default) database **`willow_20`**. Connection precedence:
