...
- **GAP-002 — DB bootstrapping duplicated**: `schema.sql` exists, but there’s no single “one true setup” doc for `willow_19` vs `grove_local` naming and `.env` vs `export` usage.
