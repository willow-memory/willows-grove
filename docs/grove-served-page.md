...
| `GROVE_SERVE_PORT` | `8766` | [`grove_serve.py`](../grove_serve.py) | Bind port, loopback ONLY — never fronted by a tunnel (D4). `8767` is `grove-mcp-serve`, `8765` is willow-mcp's code default (held here by the Nestor UI) and `8768` is willow-mcp `--serve`'s operator bind on this box; do not collide. |
