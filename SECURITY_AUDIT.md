...
| `tests/test_ci_security_grep_docstring_matches_pattern.py` | Grove v0.9 PR 12 (Loki finding m34) — pins that `scripts/ci-security-grep.sh`'s docstring pattern-list matches PATTERN. INVARIANTS.md §10. | Out of scope — test code; not shipped and not reachable at runtime |
| `tests/test_ci_playwright_install_command.py` | Grove v0.9 PR 12 (Loki finding m35) — pins §10 that the Playwright install step uses `npx --yes playwright install --with-deps chromium` verbatim. | Out of scope — test code; not shipped and not reachable at runtime |
| `tests/test_release_connection_poison.py` | Grove v0.9 PR 12 (Loki finding m38) — pins that grove_db.release_connection closes a connection whose rollback raised (not returned to the pool). | Out of scope — test code; not shipped and not reachable at runtime |
| `tests/test_grove_rename_soil_partial_failure.py` | Grove v0.9 PR 12 (Loki finding m38) — pins that grove_rename_channel reports partial failure when SOIL cursor migration fails (not fake success). | Out of scope — test code; not shipped and not reachable at runtime |
| `tests/test_grove_serve_env_validation.py` | Grove v0.9 PR 12 (Loki finding m37) — pins that grove_serve refuses non-numeric `GROVE_SERVE_PORT` and malformed `GROVE_SERVE_HOST` with an operator-legible refusal + non-zero exit. | Out of scope — test code; not shipped and not reachable at runtime |
| `tests/test_ci_hashfiles_guards_removed.py` | Grove v0.9 PR 12 (Loki finding m36) — pins §10 that the ollama and willow-mcp e2e workflow steps run through a fail-loud wrapper that distinguishes `.gitkeep`-only scaffold from py-files-with-no-tests (the silent no-op case). INVARIANTS.md §10. | Out of scope — test code; not shipped and not reachable at runtime |
| `scripts/run_test_dir_or_fail.sh` | Grove v0.9 PR 12 (Loki finding m36) — bash wrapper that pytest-runs a directory when any `test_*.py` file with a `def test_` exists; fails loud when `.py` files are present with no test functions (silent-no-op prevention); no-ops when the directory is `.gitkeep`-only. No writes, no network. INVARIANTS.md §10. | Reviewed |
| `tests/test_makefile_targets_present.py` | Grove v0.9 PR 12 (Loki finding m33) — pins that every Makefile target either invokes a script that exists or is a no-op. Prevents 'No such file or directory' failures when an operator runs a documented target. INVARIANTS.md §6. | Out of scope — test code; not shipped and not reachable at runtime |
| `tests/test_persona_registry_inline_shim_opt_in.py` | Grove v0.9 PR 12 (Loki finding m32) — pins §8 that `<grove-persona-registry>` only reads the inline JSON shim when the explicit `data-fixture` attribute is set; default (no attribute) always fetches /api/personas. Harness carries the opt-in attribute. INVARIANTS.md §8. | Out of scope — test code; not shipped and not reachable at runtime |
| `tests/test_grove_html_no_hardcoded_status.py` | Grove v0.9 PR 12 (Loki finding m31) — pins §8 that `grove_html._TOP_STRIP` does not render 'standing' or 'grove stable' as static visible text (they may still appear in CSS class/id attributes). INVARIANTS.md §8. | Out of scope — test code; not shipped and not reachable at runtime |
